...
# --- CHAT LOGIC ---
if user_prompt := st.chat_input("Search..."):
    st.session_state.messages.append({"role": "user", "content": user_prompt})
    
    with st.spinner("Analyzing..."):
        query_plan = gemini_generate_mongodb_query(user_prompt)
        db_results = execute_queries(query_plan)
        summary = get_conversational_summary(user_prompt, db_results)
        
        # We store the summary and the data results in the message state
        st.session_state.messages.append({
            "role": "assistant", 
            "content": summary, 
            "data": db_results,
            "query_text": user_prompt # Save this for deep search button
        })
    st.rerun()
    ...
    with st.chat_message(msg["role"]):
        if msg["content"] == "NO_RESULTS_FOUND":
            ...
            if st.button("🚀 Run Deep Search"):
                with st.spinner("Gemini is reading the local archives..."):
        ...
        # If there is data to display in tabs
